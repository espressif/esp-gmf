...
| 芯片 | 格式 | Internal (KB) | PSRAM (KB) | Total (KB) | CPU (%) | 启播 (ms) | 复播 (ms) |
|------|------|---------------|------------|------------|---------|-----------|-----------|
...
### ESP Player 与 ESP-Audio 播放性能对比

`esp_audio` 是 ESP-ADF v2.x 版本的播放音频的播放器。下表使用 ESP32-S3，在相同格式和测试条件下对比 `esp_audio` 与 `esp_player` 的资源开销、启播和复播时间。表格中 `esp_player` 行括号内容表示相比 `esp_audio` 的降幅。

| 格式 | 组件 | Internal (KB) | PSRAM (KB) | Total (KB) | CPU (%) | 启播 (ms) | 复播 (ms) |
|------|------|---------------|------------|------------|---------|-----------|-----------|
| MP3 | esp_audio | 19.0 | 109.1 | 128.1 | 8 | 74 | 74 |
| MP3 | esp_player | 8.8（降低 53.7%） | 91.8（降低 15.9%） | 100.6（降低 21.5%） | 5（降低 37.5%） | 32（降低 56.8%） | 27（降低 63.5%） |
| AAC | esp_audio | 35.7 | 182.9 | 218.6 | 5 | 80 | 80 |
| AAC | esp_player | 10.1（降低 71.7%） | 106.4（降低 41.8%） | 116.5（降低 46.7%） | 3（降低 40.0%） | 19（降低 76.3%） | 6（降低 92.5%） |
...
说明：
- H264 在上述芯片上均使用软件解码。
- MJPEG 在 ESP32-S3 上使用软件解码，在 ESP32-S31 和 ESP32-P4 上使用硬件解码。
- 视频播放对内存和算力要求较高，建议使用带 PSRAM 的 ESP32-S31、ESP32-P4 等芯片。若需要较高分辨率的 MJPEG 视频，推荐使用支持硬件 MJPEG 解码的 ESP32-S31、ESP32-P4。
