...
#define _SET_BITS(group, bit)  video_render_event_grp_set_bits((video_render_event_grp_handle_t)group, bit)
// Need manual clear bits
#define _WAIT_BITS(group, bit)                                                                                  \
    video_render_event_grp_wait_bits((video_render_event_grp_handle_t)group, bit, VIDEO_RENDER_MAX_LOCK_TIME);  \
    video_render_event_grp_clr_bits(group, bit)

#define _MUTEX_LOCK(mutex)    video_render_mutex_lock((video_render_mutex_handle_t)mutex, VIDEO_RENDER_MAX_LOCK_TIME)
#define _MUTEX_UNLOCK(mutex)  video_render_mutex_unlock((video_render_mutex_handle_t)mutex)
...
WEAK data_q_t *data_q_init(int size)
{
    data_q_t *q = video_render_calloc(1, sizeof(data_q_t));
    if (q == NULL) {
        return NULL;
    }
    q->buffer = video_render_malloc(size);
    q->lock = video_render_mutex_create();
    q->write_lock = video_render_mutex_create();
    video_render_event_grp_create(&q->event);
...
WEAK void data_q_wakeup(data_q_t *q)
{
    if (q && q->lock) {
        _MUTEX_LOCK(q->lock);
        q->quit = 1;
...
WEAK int data_q_consume_all(data_q_t *q)
{
    if (q && q->lock) {
        _MUTEX_LOCK(q->lock);
...
WEAK void data_q_deinit(data_q_t *q)
{
    if (q == NULL) {
        return;
    }
    if (q->lock) {
        video_render_mutex_destroy((video_render_mutex_handle_t)q->lock);
    }
    if (q->write_lock) {
        video_render_mutex_destroy((video_render_mutex_handle_t)q->write_lock);
    }
    if (q->event) {
        video_render_event_grp_destroy(q->event);
    }
    if (q->buffer) {
        video_render_free(q->buffer);
    }
    video_render_free(q);
}
...
WEAK int data_q_get_avail(data_q_t *q)
{
    if (q == NULL) {
        return 0;
    }
...
WEAK void *data_q_get_buffer(data_q_t *q, int size)
{
    int avail = 0;
    size += DATA_Q_ALLOC_HEAD_SIZE;
    if (q == NULL || size + DATA_Q_ALLOC_HEAD_SIZE > q->size) {
        return NULL;
    }
...
WEAK void *data_q_get_write_data(data_q_t *q)
{
    if (q == NULL) {
        return NULL;
    }
...
WEAK int data_q_send_buffer(data_q_t *q, int size)
{
    int ret = -1;
    if (q == NULL) {
        return -1;
...
WEAK bool data_q_have_data(data_q_t *q)
{
    int has_data = false;
    if (q == NULL) {
        return has_data;
...
WEAK int data_q_read_lock(data_q_t *q, void **buffer, int *size)
{
    int ret = -1;
    if (q == NULL) {
        return -1;
...
WEAK int data_q_peek_unlock(data_q_t *q)
{
    int ret = -1;
    if (q) {
        _MUTEX_LOCK(q->lock);
...
WEAK int data_q_read_unlock(data_q_t *q)
{
    int ret = -1;
    if (q) {
        _MUTEX_LOCK(q->lock);
...
WEAK int data_q_rewind(data_q_t *q, int blocks)
{
    int ret = -1;
    if (q == NULL) {
        return ret;
...
WEAK int data_q_query(data_q_t *q, int *q_num, int *q_size)
{
    if (q == NULL || q_num == NULL || q_size == NULL) {
        return -1;
    }
    _MUTEX_LOCK(q->lock);
    *q_num = *q_size = 0;
