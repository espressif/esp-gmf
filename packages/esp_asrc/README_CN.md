...
- **ESP32-P4 芯片版本**：软件路径的汇编优化要求 **芯片版本 ≥ 3.0**。若芯片版本 **< 3.0** ，运行软件 `rate_cvt` 时可能触发 **Guru Meditation Error**（`Core 0 panic'ed (Unknown). Exception was unhandled.`）。此类芯片请使用 **ESP-ASRC 版本 < 1.1.0**（无该汇编优化依赖），或改用硬件 ASRC 路径（若可用）。
- **ESP32-S31 核亲和性**：使用软件 `rate_cvt` 且开启 `ESP_AUDIO_EFFECTS_S31_USE_ASM` 时，任务默认固定在 **核 1**。若需自定义任务绑核，请关闭 `ESP_AUDIO_EFFECTS_S31_USE_ASM`。
