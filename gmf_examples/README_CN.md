...
| [pipeline_http_download_to_sdcard](./basic_examples/pipeline_http_download_to_sdcard) | 通过 HTTP 协议下载文件并保存到 SD 卡 | - copier | HTTP 下载 -> 文件复制器 -> SD 卡存储 |
| [pipeline_record_http](./basic_examples/pipeline_record_http) | 录制音频上传到 HTTP 服务器 | - aud_enc | 输入设备 -> 编码器 -> HTTP 服务器 |
| [pipeline_audio_effects](./basic_examples/pipeline_audio_effects) | 播放音频并应用多种特效（ALC, Sonic, EQ, Fade, DRC, MBC）和混音功能 | - aud_dec<br>- aud_alc<br>- aud_sonic<br>- aud_eq<br>- aud_fade<br>- aud_drc<br>- aud_mbc<br>- aud_mixer | Flash -> 解码器 -> 音频特效/混音器 -> 输出设备 |

| [pipeline_loop_play_no_gap](./basic_examples/pipeline_loop_play_no_gap) | 使用策略函数实现无缝循环播放音乐 | - aud_dec<br>- aud_ch_cvt<br>- aud_bit_cvt<br>- aud_rate_cvt | SD卡 -> 单管道解码 -> 音频处理 -> 输出设备 |
