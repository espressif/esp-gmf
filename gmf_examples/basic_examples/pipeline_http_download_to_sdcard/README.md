...
This example demonstrates how to implement high-speed downloading and local saving of network files based on the GMF framework, while also supporting the measurement of download speed.

**Core Process**: The `copier` element downloads network files through `io_http`, then uses `io_file` to write the files to the microSD card.

## Environment Requirements

### ESP-IDF Version

This example supports ESP-IDF release/v5.3 and later versions.

### Hardware Preparation

- A microSD card (downloaded files will be automatically stored on it)
- ESP32 development board that supports microSD card

## Configuration

### Default Configuration

- **Download URL**: [https://dl.espressif.com/dl/audio/ff-16b-2c-44100hz.opus](https://dl.espressif.com/dl/audio/ff-16b-2c-44100hz.opus)
- **Storage filename**: `ff-16b-2c-44100hz.opus`
- **Configuration parameters**: Can be customized through `http_url` and `save_url`

> **Tip**: When the `ONLY_ENABLE_HTTP` option is enabled, only file downloading is performed without writing to the microSD card, which can be used for network connection and download performance testing.

## Build and Flash

### Environment Preparation

Before compiling, please ensure that the ESP-IDF development environment is properly configured. If not yet configured, please run the following commands in the ESP-IDF root directory:
...
For detailed ESP-IDF configuration and usage guide, please refer to the [ESP-IDF Programming Guide](https://docs.espressif.com/projects/esp-idf/en/latest/esp32s3/index.html).
...
After the program starts, it will automatically execute the following process:
1. Mount the microSD card and connect to the WiFi network
2. Download files from the specified URL
3. Store the downloaded files on the microSD card
4. Stop and exit automatically after completion

### Running Example

The program will output detailed execution logs during runtime, including system startup, network connection, file download, and storage process.   

Here is the complete running output(using ESP32-S3 as an example):
