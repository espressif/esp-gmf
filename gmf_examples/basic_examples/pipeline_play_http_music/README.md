...
This example demonstrates how to play online(NOT SUPPORT m3u8 URL) music using ESP-GMF. It fetches an MP3 file via HTTP I/O, decodes it with audio decoder, applies basic audio effects, and finally outputs the audio through the CODEC_DEV_TX I/O.
...
- Enter the location where the HTTP music playback example project is stored
...
- After the example starts running, it will automatically play the music files transmitted by the HTTP, stop and exit after playback is complete, with the following output:

```c
I (1045) PLAY_HTTP_MUSIC: [ 1 ] Mount peripheral
E (1104) i2c.master: this port has not been initialized, please initialize it first
I (1112) CODEC_INIT: Set master handle 0 0x3c1d236c
...
Run `menuconfig > Component config > ESP-TLS` , and enabled `Allow potentially insecure options > Skip server verification by default`
