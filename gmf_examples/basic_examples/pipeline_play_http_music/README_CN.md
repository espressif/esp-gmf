...
本例程介绍了使用 ESP-GMF 在线播放音乐（不支持 m3u8 URL）。通过 HTTP I/O 获取 MP3 文件，使用音频解码器解码，然后进行基本的音效处理，最后通过 CODEC_DEV_TX I/O 输出音频。
...
- 例程开始运行后，自动播放 HTTP 获取的音乐文件，播放完成后停止并退出，打印如下：
...
如果您的日志有如下的错误提示，请修改 `sdkconfig` 中对于 SSL server verification 设置:

运行 `menuconfig > Component config > ESP-TLS` 勾选 `Allow potentially insecure options > Skip server verification by default`
