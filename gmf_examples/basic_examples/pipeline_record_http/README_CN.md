...
本例程介绍了通过 CODEC_DEV_RX IO 获取 codec 录制的声音，经 encoder 元素编码后，通过 HTTP IO 将编码数据上传到 HTTP 服务器。

本例程支持将录音编码为 AAC、G711A、G711U、AMRNB、AMRWB、OPUS、ADPCM、LC3、SBC 和 PCM 音频格式，默认使用 PCM 格式。
...
本例程需要搭建一个 HTTP 服务器。录音编码后的音频流会上传到 HTTP 服务器，用户可自定义 HTTP 服务器接收音频流后的处理逻辑。

本例程 `${PROJECT_DIR}/` 文件夹中提供了一个测试脚本 `server.py` 。可通过运行脚本搭建默认的 HTTP 服务器，服务器会接收音频流并保存为对应格式的文件，python 脚本命令如下：
...
```
在 `menuconfig` 中选择 `GMF APP Configuration` -> `Example Connection Configuration` -> `WiFi SSID` 和 `WiFi Password`
```

- 配置示例参数(如 HTTP 服务器地址)

```
在 `menuconfig` 中选择 `GMF Example Configuration` -> `HTTP SERVER URL`，例如 `http://192.168.1.101:8000/upload`，然后保存退出
```
...
- 本例程需要电脑和开发板连接到同一个 Wi-Fi 网络，运行脚本搭建 HTTP 服务器请参考 [预备知识](#预备知识) 一节。脚本成功运行后输出：

```
Serving HTTP on 192.168.1.101 port 8000
```

- 例程开始运行后，录音数据经过编码后将上传 HTTP 服务器，录制完成后停止退出，打印如下：
