...
- After run, export files from the SDCard and use software to check the recording:
  - When encoding is disabled and AEC runs at 8 kHz sample rate, the file name is `aec_8k_16bit_1ch.pcm` (8 kHz, 16-bit, mono)
  - When encoding is disabled and AEC runs at 16 kHz sample rate, the file name is `aec_16k_16bit_1ch.pcm` (16 kHz, 16-bit, mono)
  - When encoding is enabled, the file name is `aec.aac`
